<template>
  <div>
    <img v-bind:src="options.img" @click="editCropper()" title="点击上传图片" class="img-lg" />
...
export default {
  ...
  created() {
    ...
    let splitAvatarPath = this.avatar.split('/')
  ...
  methods: {
    ...
    beforeUpload(file) {
      if (file.type.indexOf("image/") == -1) {
        this.msgError("图片格式错误，请上传图片类型,如：jpg、png、ico后缀的图片。");
      } else {
    ...
    uploadImg() {
      this.$refs.cropper.getCropBlob(data => {
        ...
        uploadAvatar(formData).then(response => {
          if (response.code === 0) {
            this.open = false;
            this.options.img = process.env.VUE_APP_MEDIA_BASE + response.data.path;
            this.msgSuccess("图片上传成功");
            this.$emit('uploadAvatar',response.data.path);
          }
          this.visible = false;
        });
      });
